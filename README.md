...
| Leads inbox | Any **signed-in FirmLedger member** contacts a claimed (verified-owner) business from its profile — the member's account name and email are attached automatically, so the business always replies to a real account and its own address is never exposed → instant in-app + email ping to the owner → one shared thread both sides can keep talking in (the member's side lives under **Sent**, always free) → lead worked new → contacted → qualified → won/lost with private owner notes, archive and permanent delete. Exactly one email per side per conversation, then in-app notifications. **Receiving is free; reading/managing the business inbox needs Pro** — a customer is never blocked from reaching a business. The whole path is gated by `npm run test:leads-contact` (142 checks over real HTTP) |
| Owner analytics + conversion funnel | Views, website-click beacon, top locations, per-listing totals (humans only — bots/admins/owners excluded), raw events auto-purged past 400 days — plus the funnel panel: sponsored/featured impressions → views → clicks → leads → qualified → won, with contact/qualify/win rates and a plain-English headline |
| Sponsored placements + fair rotation | Clearly labelled cards on home (12-strip), directory inline, category pages, search top-2 — always matching the active filters, randomly drawn over the whole pool every visit (marquee when overflowing), every card view recorded as an impression. Fairness is enforced by `npm run test:scale` (coverage + chi-square uniformity) |
| Weekly leads report | Auto-sent when an owner's listings saw activity (quiet weeks silent): full funnel for Pro, teaser for Free. Per-owner channel choice in Settings: notification / email / both / none |
