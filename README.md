...
| Leads inbox | Any **signed-in FirmLedger member** contacts a claimed (verified-owner) business from its profile — the member's account name and email are attached automatically, so the business always replies to a real account and its own address is never exposed → instant in-app + email ping to the owner → one shared thread both sides can keep talking in (the member's side lives under **Sent**, always free) → lead worked new → contacted → qualified → won/lost with archive and permanent delete. Exactly one email per side per conversation, then in-app notifications. **Receiving is free; reading/managing the business inbox needs Pro** — a customer is never blocked from reaching a business. The whole path is gated by `npm run test:leads-contact` (142 checks over real HTTP) |
